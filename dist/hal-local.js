...
exports.LocalHalProvider = void 0;
exports.localHeuristicScore = localHeuristicScore;
/** Verdict bands kept consistent with the deployed HAL (PASS <0.4 ≤ FLAG <0.6 ≤ VETO). */
function verdictFor(halScore) {
    if (halScore >= 0.6)
        return 'VETO';
    if (halScore >= 0.4)
        return 'FLAG';
 ...
 */
function localHeuristicScore(response, options = {}) {
    const text = (response ?? '').trim();
    const overconfident = /(definitely|guaranteed|100%\b|without a doubt|absolutely certain|never fails|always works|undeniabl|proven fact)/i.test(text);
    const hedged = /(may |might |could |possibly|likely|approximately|around |estimat|i think|to my knowledge|uncertain|appears? to)/i.test(text);
    const hasEvidence = /(https?:\/\/|\bsource\b|according to|\bcit(e|ation)|\breference|\[\d+\]|doi:)/i.test(text);
    const specificUnsourced = /(\b\d{1,3}(\.\d+)?\s?%|\b(1[5-9]\d{2}|20\d{2})\b|\$\s?\d[\d,]*(\.\d+)?)/.test(text) &&
        !hasEvidence;
    const harmful = /(how to (kill|make a bomb)|build a weapon|commit suicide|exploit this vuln)/i.test(text);
    let risk = 0.15; // base prior
    if (overconfident)
        risk += 0.25;
    if (specificUnsourced)
        risk += 0.25;
    if (!hedged)
        risk += 0.05;
    if (hedged)
    ...
    if (harmful)
        risk += 0.4;
    if (text.length < 8)
        risk += 0.25; // empty/trivial → uncertain
...
class LocalHalProvider {
    constructor() {
        this.name = 'local-heuristic-v0';
    }
    async score(response, options = {}) {
        return localHeuristicScore(response, options);
    }
}
exports.LocalHalProvider = LocalHalProvider;

...
/**
 * Owner-only write. Refuse symlink or nlink>1 at dest, then write a new inode
 * (O_EXCL temp + rename) so a planted hard link cannot capture the API key.
 */
function writePrivate(file, text) {
  ...
  try {
    ...
    if (st.nlink > 1) {
      throw new Error(`${file} is a hard link; refusing to write credentials`);
    }
  } catch (e) {
  ...
  try {
    ...
    try { fs.unlinkSync(tmp); } catch { /* leftover tmp */ }
    throw e;
  }
}

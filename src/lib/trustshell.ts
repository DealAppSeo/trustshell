...
import {
  getRepID as readOnChainRepID,
  getReputationHistory as readOnChainHistory,
  getAttestation as readOnChainAttestation,
  resolveAgentId,
  RepIDSummary,
  FeedbackItem,
  AttestationDetails
} from './reputation';
import { X402Client, payAndEscrow as x402PayAndEscrow } from './x402-client';
...
export class TrustShell extends EventEmitter {
  ...
  async verifyOutput(response: string, options: ScoreOptions = {}): Promise<ScoreResult> {
  ...
  async audit(tableOrSessionId: string = 'hal_classifications'): Promise<AuditResult> {
  ...
  async executeA2A(options: {
    ...
    if (requestRes.status !== 402 && !requestRes.ok) {
      throw new TrustShellError(
        `Failed to create tip request: ${requestRes.status} ${requestRes.statusText}`,
        requestRes.status
      );
    }
    ...
    const offer = accepts.find((a: any) => a.scheme === 'exact');
    if (!offer) {
    ...
    if (!deliverRes.ok) {
      throw new TrustShellError(
        `Tip delivery failed: ${deliverRes.status} ${deliverRes.statusText}`,
        deliverRes.status
      );
    }
  ...
  async getRepID(agentId?: string | number): Promise<{ value: number; count: number; decimals: number }> {
    ...
    if (isTokenIdOrAddress) {
      ...
      const summary = await readOnChainRepID(targetAgentId, options);
      return {
  ...
  async presentProof(agentId?: string | number): Promise<{
    id: number;
    agentId: string;
    proofType: string;
    tierProven: string;
  ...
  async getReputationHistory(agentId?: string | number, options: any = {}): Promise<FeedbackItem[]> {
    const targetAgentId = agentId !== undefined ? String(agentId) : this.config.agentId;
    if (!targetAgentId) {
    ...
    if (isTokenIdOrAddress) {
      ...
      return readOnChainHistory(targetAgentId, readOptions);
    }
  ...
  async getAttestation(txHash: string, options: any = {}): Promise<AttestationDetails> {
    const readOptions = {
      rpcUrl: this.config.rpcUrl,
      engineUrl: this.baseUrl,
      ...options
    };
    return readOnChainAttestation(txHash, readOptions);
  }
  ...
  async evaluate(text: string, certainty: number, options: any = {}): Promise<any> {
  ...
  async report(decision: any): Promise<any> {
